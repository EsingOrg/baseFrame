...
<body>
  ...
  <div class="r_main clearfix">
    ...
    <!-- 搜索表框  -->
    <form id="dataForm" action="letterList.do" method="post">
    	<div class="main_con">
          <div class="admin_input clearfix">
	          ...
	          <p class="lo_btn"> 
	            ...
	            <shiro:hasPermission name="YYGL_XXGL_ZNX_XZ">   
	...
	<div class="main_con">
		<div class="">
		  <table width="100%" border="0" cellspacing="0" cellpadding="3" class="main_table tc">
			...
			<c:forEach items="${pageList.data.pageResult.list}" var="letter" varStatus="st">
		    <tr <c:if test="${st.count%2 eq 0 }">class="grey"</c:if>>
		      <td>${st.count}</td>
		      <td>${letter.title}</td>
		      <td>${letter.content}</td>
		      <td>${letter.count}</td>
	    	  <td>${letter.realName}</td>
	    	  <td>${letter.createTime}</td>
	    	  <td>
	    		<a class="orange" href="javascript:void(0);" onclick="openPoup(${letter.id},'yes')">查看</a>
	    		<shiro:hasPermission name="YYGL_XXGL_ZNX_ZCBJFS">
	    		<a class="orange" href="javascript:void(0);" onclick="openPoupM(${letter.id},'no')">再次编辑发送</a>
	    		</shiro:hasPermission>
	    	  </td>
		    </tr>
		    </c:forEach>
	  ...
	  <div class='paging' style="margin-top: 25px;">
			<dim:page url="letterList.do" totalCount="${pageList.data.pageResult.recordCount }" formId="dataForm" showPages="${pageList.data.pageResult.pageSize }"
 ...
 function openPoup(id, flag){
	   $.tbox.popup("toEditLetter.do?isEdit=" + flag + "&id=" + id + "&t=" + Math.random());
};
function openPoupM(id, flag){
	   $.tbox.popup("toEditLetter.do?isEdit=" + flag + "&m=m&id=" + id + "&t=" + Math.random());
};
 </script>
</body>
</html>