...
<body>
  ...
  <div class="r_main clearfix">
    ...
    <!-- 搜索表框  -->
    <form id="dataForm" action="useDetailList.do" method="post" id="dataForm">
    	<div class="main_con">
          <div class="admin_input clearfix">
	           ...
	           <p>状态： 
	          	<dim:select name="state" enumType="ExperienceStatus" headName="全部" headValue="" exclude="UNUSE,EXPIRE" cssClass="yhgl_sel"></dim:select>
	          </p> 
	          <p>开始时间： 
	          ...
	          <p class="lo_btn"> 
	            ...
	            <shiro:hasPermission name="YYGL_TYJ_SYMX_DC">
	   		    <input name="input" type="button" class="export ml10" value="导出" onclick="exportData('dataForm','exportUseDetail.do');"/>
	   		    </shiro:hasPermission>
			  </p>
		  </div>
       </div>
	...
	<div class="main_con">
		<div class="">
		  <table width="100%" border="0" cellspacing="0" cellpadding="3" class="main_table tc">
		    ...
		    <tr <c:if test="${st.count%2 eq 0 }">class="grey"</c:if>>
	    	  ...
	    	  <td>${user.source}</td>
	    	  <td>${user.weight}</td>
	    	  <td>${user.finishAmount}</td>
	    	  <td>${user.status}</td>
	    	  <td>${user.incomeDay}</td>
	    	  <td>${user.startTime}</td>
	    	  <td>${user.endTime}</td>
	    	  <td>
	    	  <shiro:hasPermission name="YYGL_TYJ_SYMX_SYXQ">
	    	  <a class="orange" href="javascript:void(0);" onclick="window.location.href='showDetail.do?experienceId=${user.experienceId}'">收益详情</a>
	    	  </shiro:hasPermission>
	    	  </td>
		    </tr>
		    </c:forEach>
	  ...
	  <div class='paging' style="margin-top: 25px;">
			<dim:page url="useDetailList.do" totalCount="${pageList.data.pageResult.recordCount }" formId="dataForm" showPages="${pageList.data.pageResult.pageSize }"
