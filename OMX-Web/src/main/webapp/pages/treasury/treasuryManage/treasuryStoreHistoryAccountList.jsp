...
<body>
	<div class="systemUser">
		<div class="r_main clearfix ">
			<!--文本框 -->
			<div class="main_title">店铺历史台账<span style="margin-left: 130px;">正欠负存</span></div>
			<div class="main_con clearfix">
				<div class="user_con  all">
					<ul class="user_now clearfix">
						...
						<li>
							<p class="yellow">
								<span class="f24">
								${data.statResult == null ? '0.000' : data.statResult.saveWeightTotal == null ? '0.000' : data.statResult.saveWeightTotal}
								</span>克
							</p>
							<p>${flag == 1 ? '累计存入' : flag == 2 ? '累计客户存入':'累计店铺存入'}</p>
						</li>
						<li>
							<p class="yellow">
								<span class="f24">
								${data.statResult == null ? '0.000' : data.statResult.takeWeightTotal == null ? '0.000' : data.statResult.takeWeightTotal}
								</span>克
							</p>
							<p>${flag == 1 ? '累计提取' : flag == 2 ? '累计客户提取':'累计店铺提取'}</p>
						</li>
						<li>
							<p class="yellow">
								<span class="f24">
								${data.statResult == null ? '0.000' : data.statResult.sendWeightTotal == null ? '0.000' : data.statResult.sendWeightTotal}
								</span>克
							</p>
							<p>累计平台已发货</p>
						</li>
						<li>
							<p class="yellow">
								<span class="f24">
								${data.statResult == null ? '0.000' : data.statResult.getWeightTotal == null ? '0.000' : data.statResult.getWeightTotal}
								</span>克
							</p>
							<p>累计平台已收货</p>
						</li>
			...
			<!--文本框 --END-->
			<!-- 搜索框 -->
			<form action="getTreasuryStoreHistoryAccountList.do" method="post"
				id="allForm">
				...
				<div class="main_con">
					<div class="admin_input clearfix">
						...
						<p class="lo_btn">
							...
							<shiro:hasPermission  name="JKGL_JKGL_DPLSTZ_QB_DC">
							<c:if test="${flag == 1}">
							<input name="input" type="button" class="export ml10" value="导出"  onclick="exportData('allForm','historyAccountExport.do')"/>
							</c:if>
							</shiro:hasPermission>
							<shiro:hasPermission  name="JKGL_JKGL_DPCLGRYW_DC">
							<c:if test="${flag == 2}">
							<input name="input" type="button" class="export ml10" value="导出"  onclick="exportData('allForm','historyAccountExport.do')"/>
							</c:if>
							</shiro:hasPermission>
							<shiro:hasPermission  name="JKGL_JKGL_DPLSTZ_ZY_DC">
							<c:if test="${flag == 3}">
							<input name="input" type="button" class="export ml10" value="导出"  onclick="exportData('allForm','historyAccountExport.do')"/>
							</c:if>
							</shiro:hasPermission>
			...
			<div class="main_con">
				<div class="menubox clearfix">
					<ul>
						<li class="${flag == 1 ? 'hover':'' }" id="all"><a href="getTreasuryStoreHistoryAccountList.do?flag=1">店铺历史全部业务</a></li>
						<li class="${flag == 2 ? 'hover':'' }" id="cus"><a href="getTreasuryStoreHistoryAccountList.do?flag=2">店铺历史处理个人业务</a></li>
						<li class="${flag == 3 ? 'hover':'' }" id="ps"><a href="getTreasuryStoreHistoryAccountList.do?flag=3">店铺历史自营业务</a></li>
					</ul>
				</div>
				<div class="lb_content_table all">
					<table width="100%" border="0" cellspacing="0" cellpadding="3"
						class="main_table tc">
							<tr class="title">
								...
								<td align="center">${flag == 1 ? '' : flag == 2 ? '客户':'店铺'}存入（克）</td>
								<td align="center">${flag == 1 ? '' : flag == 2 ? '客户':'店铺'}提取（克）</td>
							...
							<c:forEach items="${data.pageResult.list}" var="dt" varStatus="st">
								<tr>
						        	...
						        	<td>${dt.endGold}</td> 
						        	<td><a class="orange" href="accountShow.do?userName=${dt.userName }&shopName=${dt.shopName}&shopId=${dt.shopId}&flag=${flag}">查看</a></td>
								</tr>
							</c:forEach>
							<tr class="yellow" >
								<td> </td>
					...
					<div style="margin-top: 25px;" id="pageId">
						<dim:page url="getTreasuryStoreHistoryAccountList.do" totalCount="${data.pageResult.recordCount }" formId="allForm" showPages="${data.pageResult.pageSize }"
