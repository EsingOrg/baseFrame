...
dmCheck.init("#auditing");
  function suaud(){
	  ...
	  $.tbox.close();
	  //提示操作状态
	  dm.confirm("修改前平台交易金价：${nowPrice} 元/克" + "<br/>是否确认将最新交易金价修改为：<span class='red'>"+$("#amount").val()+"</span>元/克",{
		title:"确认修改信息",				//标题
		...
		callback:function(){	//按确定的执行的函数
        	$.ajax({
      		  type: "POST",
      		  url:"createTimeKingPriceSure.do",	//请求路径
      			  ...
      			  if(result.flag.code=='000000'){
      				  ...
      				  dm.alert("新建成功",{
      			        ...
      			        callback:function(){	//按确定后执行的函数
      			        	 window.location.href='<%=basePath %>treasury/getFrontTradeKingPriceList.do';
...
<div class="dialog">
  <div class="clearfix"> 
      <div class="title"><a href="javascript:;" onClick="$.tbox.close()" class="out"></a>新建金价</div>
      <div class="cotent">
       <form action="<%=basePath %>treasury/createTimeKingPrice.do" method="post" id="auditing">
       <input type="hidden" name="no" value = "${no }">
       <input type="hidden" name="token" value="${token}"/><!--  重复提交Token-->
       <div style="text-align: center;">当前交易金价：<span class="red">${nowPrice}</span>元/克</div>
         <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table">
          <tr>
            <td valign="top" width="30%" align="right">金价：</td>
            <td><input name="updatePrice" value="" id="amount" type="text" class="yhgl_input" validate="q|amount"/>元/克
            </td>
          </tr> 
        </table>
        </form>
