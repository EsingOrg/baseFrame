	...
	<div>				
			<div class="r_main clearfix ">
	          ...
	          <p>
	            ...
	            <p>
	                                        手机号：
	                 <input type="text" name="allUserPhone" class="yhgl_input" maxlength="20"/>
	           </p>
	           <p>角色：
	           ...
	           <p>交易时间：
				...
				<p>成交金价：
	              ...
	              <shiro:hasPermission name="YWGL_SJYW_MJZB_DC">
	               <input name="input" type="button" class="export ml10" value="导出" onclick="exportData('form','saleGoldExpo.do');"/>
	               </shiro:hasPermission>
	            </div>
	            </div> 
          <!--搜索表框  --END-->  
  ...
  //页面加载时调用
    ...
    //初始加载数据
 	function search(){
 		...
 		DMGold.ajax({"formId":"form","serialize":true,"url":"saleGoldListAjax.do","success":pageTagCallBack});
 	}
    //分页跳转回调
 	function pageTagCallBack(data){
 	    ...
 	    DMGold.PageTag.init({"divId":"pageTag","formId":"form","curPage":data.pageResult.pageIndex,"totalCount":data.pageResult.recordCount,
 		          "pageCount":data.pageResult.pageTotal,"url":"saleGoldListAjax.do","toPageCallBack":pageTagCallBack});
	 	//加载统计数据
	 	if(!data.statResult){
	 		$("#totalWeight").empty();
		    $("#totalAmount").empty();
