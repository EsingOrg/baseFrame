...
<html>
  <head>
  <title>卖金业务-店铺卖金</title>
  </head>
  <body>
	<div>				
	<div class="r_main clearfix ">
	   <div class="main_title">店铺卖金</div>
		<form id="form" >
	     <!--搜索表框  -->
          <div class="main_con">
	          ...
	          <p>
	           ...
	           <p>交易时间：
				...
				<p>成交金价：
	              ...
	              <shiro:hasPermission name="YWGL_SJYW_DPMJ_DC">
	               <input name="input" type="button" class="export ml10" value="导出" onclick="exportData('form','storeSaleExpo.do');"/>
	            	</shiro:hasPermission>
	            </div>
	            </div> 
          <!--搜索表框  --END-->  
...
dmCheck.init("#form");
    ...
    //初始加载数据
   	function search(){
   		...
   		DMGold.ajax({"formId":"form","serialize":true,"url":"storeSaleGoldAjax.do","success":pageTagCallBack});
   	}
    //分页跳转回调
   	function pageTagCallBack(data){
   	    ...
   	    DMGold.PageTag.init({"divId":"pageTag","formId":"form","curPage":data.pageResult.pageIndex,"totalCount":data.pageResult.recordCount,
   		          "pageCount":data.pageResult.pageTotal,"url":"storeSaleGoldAjax.do","toPageCallBack":pageTagCallBack});
      //加载统计数据
      if(!data.statResult){
    	  $("#totalWeight").empty();
          $("#totalAmount").empty();
