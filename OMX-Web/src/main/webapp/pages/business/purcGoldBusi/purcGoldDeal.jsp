  ...
  <body>
	<div >				
			<div class="r_main clearfix ">
			  <div class="main_title">买金处理</div>
			  <!--  -->
			  <div class="main_con clearfix">
          	<div class="user_con">
                <ul class="user_now clearfix">
                    <li>
                        <p class="yellow"><span class="f24" id="lastWaitWeight"></span>克</p>
                        <p>最新待处理克重</p>
                    </li>
                    <li>
                        <p class="yellow"><span class="f24" id="handleWeightTotal"></span>克</p>
                        <p>累计已处理克重</p>
                    </li>
                    <li>
                        <p class="yellow"><span class="f24" id="lastMonthWeight"></span>克</p>
                        <p>近一月已处理克重</p>
                    </li>
                </ul>
          	</div>
	          ...
	          <div class="admin_input clearfix">
	            ...
	            <p class="lo_btn">
	              ...
	              <shiro:hasPermission name="YWGL_MJYW_MJCL_XZMJCL">
	              <a onclick="popDiv('<%=basePath %>business/purcGoldBusi/purcGoldDealEdit.do?t=<%=Math.random() %>')" href="javascript:void(0)" class="btn_cs ml10"><i class="newly"></i>新增买金处理</a>
	            	</shiro:hasPermission>
	            </p>
	            </div>
	            </div> 
...
$(function(){
   ...
   $("#searBtn").click(function(){
 	  DMGold.ajax({"formId":"searchBox","serialize":true,"url":"purcGoldDealAjax.do","success":searCallBack});
   });
   //加载列表数据
   $("#searBtn").trigger('click');
   //加载统计数据
   DMGold.ajax({"formId":"searchBox","serialize":true,"url":"purcGoldDStatAjax.do","success":initStat});
});
//回调方法
function searCallBack(data){
	...
	DMGold.PageTag.init({"divId":"pageTag","formId":"searchBox","curPage":data.pageResult.pageIndex,"totalCount":data.pageResult.recordCount,
		          "pageCount":data.pageResult.pageTotal,"url":"purcGoldDealAjax.do","toPageCallBack":searCallBack});
}
//弹出框
function popDiv(url){
	   $.tbox.popup(url);
...
function initStat(stat){
	if(!stat){
		...
		$("#lastWaitWeight").append('0.000');
		$("#handleWeightTotal").append('0.000');
		$("#lastMonthWeight").append('0.000');
		$("#handleWeightBranch").append('0.000');
		return false;
	}
	 //最新待处理克重
	 $("#lastWaitWeight").empty();
