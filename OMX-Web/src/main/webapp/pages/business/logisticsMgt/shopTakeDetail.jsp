      ...
      <div class="r_main clearfix">
		...
		<div class="main_con">
            <ul class="view clearfix" >
                ...
                <c:if test="${list.singleResult.status =='SHOP_RECEIVE' }">
                <li style="color:red; ">
            ...
            <div class="btn mt30 mb30"><a href="dispatched.do?tabIndex=shop" class="btn_cs ml20">返回</a></div>
        </div>
        ...
        <form id="searchBox">
				<div class="main_con">
					...
					<input type="hidden" id="logisticId" name="logisticId" value="${list.singleResult.logisticId}">
						...
						<p>
							用户名： <input type="text" name="userName" class="yhgl_input"  maxlength="20" />
						</p>
						<p class="lo_btn">
							<input type="button" class="search" id="searBtn" value="搜索" /> 
							<input name="input" type="button" class="export ml10" value="导出" onclick="exportData('searchBox','exportShopTakeOrdersByLogistic.do');"/>
						</p>
					</div>
				</div>
				</form>
...
$(function(){
		  ...
		  $("#searBtn").click(function(){
			  DMGold.ajax({"formId":"searchBox","serialize":true,"url":"shopTakeOrdersAjax.do?logisticId="+$('#logisticId').val(),"success":ordersCallBack});
		  });
	...
	function ordersCallBack(data){
		...
		DMGold.PageTag.init({"divId":"pageTag","formId":"searchBox","curPage":data.pageResult.pageIndex,"totalCount":data.pageResult.recordCount,
			          "pageCount":data.pageResult.pageTotal,"url":"shopTakeOrdersAjax.do?logisticId="+$('#logisticId').val(),"toPageCallBack":ordersCallBack});
		
	}
	function openDetail(orderId){
		var url = "shopTakeDetail.do?id="+orderId+"&showonly=show&t=<%=Math.random() %>"
		$.tbox.popup(url);
	}
