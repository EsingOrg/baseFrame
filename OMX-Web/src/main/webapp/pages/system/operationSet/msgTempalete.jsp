...
<html>
 ...
 <!--模板 -->
	<script id="template" type="text/x-jquery-tmpl">
     {{each(index) list}}
       <tr {{if index %2 == 0}}class="grey" {{/if}}>
        ...
        <td>
			<shiro:hasPermission name="XTGL_YYSZ_DXMB_QY">
            <a href="javascript:activeToggle('{{= id}}','{{= enableFlag}}')" class="changeStatus orange">{{= enableFlag=='Y'?'停用':'启用'}}</a>
			</shiro:hasPermission>
            <a  href ="tempEditView.do?id={{= id}}&isView=false&retUrl=../../system/operationSet/msgTempalete.do&operation=show&t=<%=Math.random() %>"class="orange">查看</a>
			<shiro:hasPermission name="XTGL_YYSZ_DXMB_XG">
            <a href="tempEditView.do?id={{= id}}&isView=false&retUrl=../../system/operationSet/msgTempalete.do&key={{= key}}&t=<%=Math.random() %>"  class="orange">修改</a>
			</shiro:hasPermission>
        </td>
        </tr>
      {{/each}}
    ...
    <script type="text/javascript">
        ...
        function search(){
     	   DMGold.ajax({"formId":"form","serialize":true,"url":"msgTempaleteAjax.do","success":pageTagCallBack});
     ...
     //分页跳转回调
    	...
    	$("#grid").empty();
   		...
   		DMGold.PageTag.init({"divId":"pageTag","formId":"form","curPage":data.pageResult.pageIndex,"totalCount":data.pageResult.recordCount,
   			          "pageCount":data.pageResult.pageTotal,"url":"msgTempaleteAjax.do","toPageCallBack":pageTagCallBack});
    	}
    //弹出框
    function popDiv(url){
 	   $.tbox.popup(url);
