...
$(function(){
    var categories1 = ${pageList.data.singleResult.categorieData}; //X轴数据
    var data1 = [{name: series[0].name,data: series[0].data}, 
                 {name: series[1].name,data: series[1].data}];	//收入/支出数据
	 $('#line').highcharts({
          title: {
              text: '<strong>平台收入支出统计曲线图</strong>'
          },
...
<script src="<%=basePath%>js/exporting.js" charset="utf-8"></script>
	<div class="clearfix">
		<!--搜索表框  -->
		<form action="incomeAndExpenseTrend.do" method="post" id="submitForm">
