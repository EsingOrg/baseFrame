...
<body>
  ...
  <div class="r_main clearfix">
    ...
    <div class="main_con"> 
	  ...
	  <div class="menubox clearfix">
		<ul>
		  <li id="one1" onclick="window.location.href='shopPerShow.do?key=${key}&name=${name }';" class="${tabIndex == '1' ? 'hover' : ''}">店铺信息</li>		  
		  <li id="one4" onclick="window.location.href='shopPerShowExclog.do?key=${key}&name=${name }';" class="${tabIndex == '6' ? 'hover' : ''}">店铺操作人</li> 
		  <li id="one2" onclick="window.location.href='shopPerShowBuylog.do?key=${key}&name=${name }';" class="${tabIndex == '2' ? 'hover' : ''}">买金记录</li>
		  <li id="one3" onclick="window.location.href='shopPerShowSalelog.do?key=${key}&name=${name }';" class="${tabIndex == '3' ? 'hover' : ''}">卖金记录</li> 
		  <li id="one4" onclick="window.location.href='shopPerShowKeeplog.do?key=${key}&name=${name }';" class="${tabIndex == '4' ? 'hover' : ''}">存金记录</li> 
		  <li id="one3" onclick="window.location.href='shopPerShowDiplog.do?key=${key}&name=${name }';" class="${tabIndex == '5' ? 'hover' : ''}">提金记录</li> 
		  
		</ul>
		<div class="clearfix"><a href="javascript:void(0);" onclick="window.location.href='shopPerShow.do?key=${key}&name=${name}';" class="btn_cs fr mr20">返回</a></div>
	  </div>
	  <!-- TAB--END -->
	  
	  <!-- TAB1 -->
	  <div class="menu_bd clearfix" id="con_one_1" style="display:${tabIndex == '1' ? '' : 'none'};">
	  
		<form action="shopPerEdit.do" method="post" id="editForm" enctype="multipart/form-data">
		<input type="hidden" name="shopId" value="${key}"/>
		<input type="hidden" name="shopUserName" value="${name}"/>
		<input type="hidden" name="flag" value="${tempdata.data.singleResult.flag}"/>
		<input type="hidden" name="token" value="${token}"/><!--  重复提交Token-->
		<div class="pat_title clearfix">注册信息</div>
		<ul class="part_center clearfix">
		  <li><div class="til"><span class="red">*</span>店铺名称：</div><div class="info"><input type="text" name='name' id="shopName" onblur="isExistShopName(this.value)" validate="q|leng" leng="1,20" class="yhgl_input w200" value="${tempdata.data.singleResult.name }"/></div></li>
		  <c:if test="${tempdata.data.singleResult.flag == 'N'}">
			  <li><div class="til"><span class="red">*</span>营业执照登记注册号：</div><div class="info"><input type="text" name="licenceNo" id="textfield" validate="q|leng" leng="1,15" reg="/^[a-zA-Z0-9]{1,15}$/" warning="格式不正确" maxlength="15" class="yhgl_input w200" value="${tempdata.data.singleResult.licenceNo}"/></div></li>
		  ...
		  <li><div class="til"><span class="red">*</span>法人身份证：</div><div class="info"><input type="text" name="corporateIdcard" id="shopIdCard"  validate="q|idCard" class="yhgl_input w200" value="${tempdata.data.singleResult.corporationCardId}"/></div></li>   
		  <li><div class="til"><span class="red">*</span>手机号：</div><div class="info"><input type="text" name="phone" id="shopPhone" validate="q|m"  class="yhgl_input w200" value="${tempdata.data.singleResult.phone}"/></div></li>
		  <li><div class="til"><span class="red">*</span>邮箱：</div><div class="info"><input type="text" name="email" id="shopEmail" validate="email"  class="yhgl_input w200" value="${tempdata.data.singleResult.email}"/></div></li>              
		</ul>  
		<div class="pat_title clearfix">基本信息</div>
		<ul class="part_center clearfix">
		  
		  <li><div class="til"><span class="red">*</span>店铺地址：</div><div class="info"><input type="text" name="address" validate="q|leng" leng="2,50" id="textfield" class="yhgl_input w200" value="${tempdata.data.singleResult.address}"/></div></li>
		  <li><div class="til"><span class="red">*</span>营业时间：</div><div class="info"><input type="text" name="startTime" validate="q|leng"  reg="/^(0\d{1}|1\d{1}|2[0-3]):([0-5]\d{1})$/" warning="格式不正确,请输入正确的时间!"  leng="0,5"  id="textfield" class="yhgl_input w100" value="${tempdata.data.singleResult.startTime}"/>&nbsp;到&nbsp;<input type="text" name="endTime" id="textfield" validate="q|leng" leng="0,5"  reg="/^(0\d{1}|1\d{1}|2[0-3]):([0-5]\d{1})$/" warning="格式不正确,请输入正确的时间!"  class="yhgl_input w100" value="${tempdata.data.singleResult.endTime}"/></div></li>
		  ...
		  <li><div class="til">备注：</div><div class="info"><textarea name="remark" cols="40" rows="5" validate="leng" leng="0,500" maxlength="200" class="border">${tempdata.data.singleResult.remark}</textarea></div></li>
		</ul>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="main_table mb20">
			<thead>
             ...
             <c:forEach items="${cotman.data.list}" var="user" varStatus="st">
             	<tr>
                    ...
                    <td align="center"><a href="javascript:void(0);" onclick="openUpdateLinkman(${user.id},${key},'${user.name }','${user.phone }','${user.remark }')" class="yellow">修改</a><a href="javascript:void(0);" onclick="delLinkMan(${user.id},${key})" class="yellow">删除</a></td>
               </tr>
             </c:forEach>
             </tbody>
           </table>
          ...
          <li style="display: ${tempdata.data.singleResult.flag == 'N' ? '' : 'none'}">
          ...
          <li style="display: ${tempdata.data.singleResult.flag == 'N' ? '' : 'none'}">
         	  ...
         	  <li style="display: ${tempdata.data.singleResult.flag == 'Y' ? '' : 'none'}">
        ...
        <div class="btn mt30 mb30"><a href="javascript:void(0);" onclick="modifyPerUserSubmit();" class="btn_cs">保存</a><a href="javascript:void(0);" onclick="window.location.href='shopPerShow.do?key=${key}&name=${name}';"class="btn_cs ml20">取消</a></div>        
         </form> 
      </div>
      <!-- TAB1--END -->
  ...
  function modifyPerUserSubmit(){
  	if((obj = dmCheck.returnObj("#editForm")))	return obj.focus();//校验输入数据格式是否正确
  	//店铺名称是否存在
  	if(isExistShopName($("#shopName").val())){
  		return;
  	}
  	//是否三证合一
  	var flag='${tempdata.data.singleResult.flag}';
  	if("Y"==flag){
  ...
  function getAllProvince(){
	...
	function changeProvince(pid){
	  ...
	  function openUpdateLinkman(id,shopid,name,tel,remark){
		  ...
		  $("#contactMan").html("<input type='text' name='name' class='yhgl_input' validate='q|leng' leng='2,5' value='"+name+"'/>");//店铺联系人
		  $("#contactTel").html("<input type='text' name='phone' class='yhgl_input' validate='q|m' value='"+tel+"'/>");//电话
		  $("#remark").html('<textarea name="remark" cols="30" rows="3" validate="leng" leng="0,50" maxlength="50" class="border">'+remark+'</textarea>');//备注
		  $("#manid").val(id);//联系人id
	  ...
	  function isExistShopName(shopName){
		  ...
		  if(shopName != ''){
			  $.ajax({
					...
					data:{"shopFullName":shopName,"shopId":'${key}'},
					success:function(msg){
						if(!msg.flag && msg.match("<script>")){
