...
<body>
	<div class="systemUser">
		<div class="r_main clearfix ">
			<!-- 搜索框 -->
			<form action="<%=basePath%>finance/getFinancePlatformTradingRecord.do"
				method="post" id="dataForm">
				<div class="main_con">
					<div class="admin_input clearfix">
						<p>
							类型： 
 							 <dim:select name="type" enumType="TransType" headName="全部" headValue="" exclude="RECHARGE,RECHARGE_OFFLINE,BORROW_MGMT,WITHDRAW,BUY,SALE,SAVE,TAKE,CHANGE,CHANGE_SAVE,CHANGE_TAKE" cssClass="yhgl_sel"></dim:select>
						</p>
						<p>
							时间： <input type="text" name="startTime"
								value="${transSeq.startTime}" class="yhgl_input date"
							...
							<span class="mr5">至</span> <input type="text" name="endTime"
								value="${transSeq.endTime}" class="yhgl_input date"
								class="Wdate" id="endDate"
								onclick="WdatePicker({readOnly:true,minDate:'#F{$dp.$D(\'startDate\')}'})" />
						</p>
						<p class="lo_btn">
							<input type="submit" class="search" value="搜索" /> 
							<shiro:hasPermission name="CWGL_JYJL_PTJYJL_DC">
							<input name="input" type="button" class="export ml10" value="导出" onclick="exportData('dataForm','exportPlatformRecord.do');"/>
							</shiro:hasPermission>
						</p>
					</div>
				</div>
			...
			<div class="main_con">
				...
				<div style="margin-top: 25px;">
					<dim:page url="getFinancePlatformTradingRecord.do" totalCount="${pageList.data.pageResult.recordCount }" formId="dataForm" showPages="${pageList.data.pageResult.pageSize }"
